  ...
  (let ((classes nil)
    ...
    (dolist (lifted lambdas)
      (let* ((name (sanitize-identifier (string (car lifted))))
             ...
             (opt-params (ast-lambda-optional-params lambda-node))
             (n-optional (length opt-params))
             (key-params (ast-lambda-key-params lambda-node))
             (allow-other-keys (ast-lambda-allow-other-keys lambda-node))
