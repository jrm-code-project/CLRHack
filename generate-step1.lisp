...
(register-primitive-step1 '("%CAR" "CAR")
  (lambda (node operands)
    (declare (ignore node))
    (mapc #'generate-step1 operands)
    (let ((null-label (sanitize-identifier (string (gensym "CAR_NULL"))))
      (end-label (sanitize-identifier (string (gensym "CAR_END")))))
  (list (il:dup)
    (il:brfalse null-label)
...
(register-primitive-step1 '("%CDR" "CDR")
  (lambda (node operands)
    (declare (ignore node))
    (mapc #'generate-step1 operands)
    (let ((null-label (sanitize-identifier (string (gensym "CDR_NULL"))))
      (end-label (sanitize-identifier (string (gensym "CDR_END")))))
  (list (il:dup)
    (il:brfalse null-label)
    (il:castclass "[LispBase]Lisp.List/ListCell")
...
(defmethod generate-step1 ((node ast-lambda))
  (let* ((req-params (ast-lambda-params node))
         ...
         (opt-supplied-names (let ((results nil))
                               (dolist (opt optionals)
                                 (when (third opt) (push (third opt) results)))
                               (nreverse results)))
...
(defmethod generate-step1 ((node ast-clr-call-virt))
  ...
  (let* ((arg-types (or (ast-clr-call-virt-arg-types node)
    ...
    (let* ((args-eval-code nil)
           ...
           (arg-temps (mapcar (lambda (a)
                                (declare (ignore a))
                                (register-local (string (gensym "CLR_VIRT_ARG"))))
                              (ast-clr-call-virt-arguments node))))
      (dolist (temp (reverse arg-temps))
        (push (il:stloc temp) args-eval-code))
      (setf (ast-basic-block node)
            (append args-eval-code
                    (list (il:castclass class-name))
                    (mapcar #'il:ldloc arg-temps)
...
(defmethod generate-step1 ((node ast-unwind-protect))
  ...
  (setf (ast-unwind-protect-extra-temps node)
        (let ((results nil))
          (do ((i 1 (1+ i)))
              ((>= i 64))
            (push (register-local (string (gensym (format nil "UWP_V~D" i)))) results))
          (nreverse results)))
    ...
    (when (> n 1)
      (setf (ast-values-temps node)
            (let ((results nil))
              (do ((i 1 (1+ i)))
                  ((>= i (min n 64)))
                (push (register-local (gensym (format nil "VALUES_TEMP_~D" i))) results))
              (nreverse results))))
...
(defmethod generate-step1 ((node ast-multiple-value-prog1))
  ...
  (setf (ast-multiple-value-prog1-extra-temps node)
        (let ((results nil))
          (do ((i 1 (1+ i)))
              ((>= i 64))
            (push (register-local (string (gensym (format nil "MV_PROG1_V~D" i)))) results))
          (nreverse results)))
